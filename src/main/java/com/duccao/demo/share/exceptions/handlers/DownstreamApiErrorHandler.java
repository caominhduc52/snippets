...
public class DownstreamApiErrorHandler {
          ...
          .flatMap(
              apiError -> {
                if (log.isDebugEnabled()) {
                  log.debug(
                      "message=\"Error occurs with commandName={}, errorId={}, statusCode={}\"",
                      commandName,
                      apiError.getErrorId(),
                      httpStatusCode);
                }
  ...
  private Mono<? extends Throwable> buildErrorResponse(
      ErrorDetails apiError, HttpStatusCode httpStatusCode) {
